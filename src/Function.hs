...
data Type = T_Int | T_Double | T_Bool | T_String | T_Func | T_BinaryOp | T_UnaryOp | T_FunctionOp | T_PreList | T_List Type | T_EmptyList | T_Tuple [Type] | T_Print | T_Function { args_t :: [Type], return_t :: Type } | T_Unknown | T_Type | T_Error deriving (Eq, Show)
...
type BinaryOp_T = TWrd -> TWrd -> TWrd
type UnaryOp_T = TWrd -> TWrd
type FunctionOp_T = (Int, TExp -> TWrd)
data Op_T = BinOp_T BinaryOp_T | UnOp_T UnaryOp_T | FuncOp_T FunctionOp_T

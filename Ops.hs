...
_print_t :: Wrd -> Wrd
_print_t (Type T_Error) = Type T_Error
_print_t (Type _) = Type T_Print
...
_mul_t :: Wrd -> Wrd -> Wrd
_mul_t (Type T_Double) (Type T_Double) = Type T_Double
_mul_t (Type T_Int) (Type T_Int) = Type T_Int
_mul_t (Type T_Int) (Type T_Double) = Type T_Double
_mul_t (Type T_Double) (Type T_Int) = Type T_Double
...
_div_t :: Wrd -> Wrd -> Wrd
_div_t (Type T_Double) (Type T_Double) = Type T_Double
_div_t (Type T_Int) (Type T_Int) = Type T_Double
_div_t (Type T_Int) (Type T_Double) = Type T_Double
_div_t (Type T_Double) (Type T_Int) = Type T_Double
...
_add_t :: Wrd -> Wrd -> Wrd
_add_t (Type T_Double) (Type T_Double) = Type T_Double
_add_t (Type T_Int) (Type T_Int) = Type T_Int
_add_t (Type T_Int) (Type T_Double) = Type T_Double
_add_t (Type T_Double) (Type T_Int) = Type T_Double
_add_t (Type T_String) (Type T_String) = Type T_String
_add_t (Type (T_List t1)) (Type (T_List t2)) =
    if t1 == t2 then Type (T_List t1) else Err $ "`+`: Type mismatch of lists: t1=" ++ (show t1) ++ ", t2=" ++ (show t2)
...
_sub_t :: Wrd -> Wrd -> Wrd
_sub_t (Type T_Double) (Type T_Double) = Type T_Double
_sub_t (Type T_Int) (Type T_Int) = Type T_Int
_sub_t (Type T_Int) (Type T_Double) = Type T_Double
_sub_t (Type T_Double) (Type T_Int) = Type T_Double
_sub_t t_x t_y = Err $ "`-`: Illegal input type: t_x=" ++ (show t_x) ++ ", t_y=" ++ (show t_y)

_succ_t :: Wrd -> Wrd
_succ_t (Type T_Int) = Type T_Int
_succ_t (Type T_Double) = Type T_Double
...
_eq_t :: Wrd -> Wrd -> Wrd
_eq_t (Type t1) (Type t2) =
    if t1 == t2 then Type T_Bool else Err $ "`=`: Type mismatch of both sides: LHS=" ++ (show t1) ++ ", RHS=" ++ (show t2)

_eq :: Op
_eq = BinOp (\ a b -> Bool (a == b))

_neq_t :: Wrd -> Wrd -> Wrd
_neq_t (Type t1) (Type t2) =
    if t1 == t2 then Type T_Bool else Err $ "`!=`: Type mismatch of both sides: LHS=" ++ (show t1) ++ ", RHS=" ++ (show t2)
...
_gt_t :: Wrd -> Wrd -> Wrd
_gt_t (Type T_Double) (Type T_Double) = Type T_Bool
_gt_t (Type T_Int) (Type T_Int) = Type T_Bool
_gt_t (Type T_Double) (Type T_Int) = Type T_Bool
_gt_t (Type T_Int) (Type T_Double) = Type T_Bool
_gt_t (Type t1) (Type t2) = Err $ "`>`: Illegal input type: LHS=" ++ (show t1) ++ ", RHS=" ++ (show t2)
...
_geq_t :: Wrd -> Wrd -> Wrd
_geq_t (Type T_Double) (Type T_Double) = Type T_Bool
_geq_t (Type T_Int) (Type T_Int) = Type T_Bool
_geq_t (Type T_Double) (Type T_Int) = Type T_Bool
_geq_t (Type T_Int) (Type T_Double) = Type T_Bool
_geq_t (Type t1) (Type t2) = Err $ "`>=`: Illegal input type: LHS=" ++ (show t1) ++ ", RHS=" ++ (show t2)
...
_lt_t :: Wrd -> Wrd -> Wrd
_lt_t (Type T_Double) (Type T_Double) = Type T_Bool
_lt_t (Type T_Int) (Type T_Int) = Type T_Bool
_lt_t (Type T_Double) (Type T_Int) = Type T_Bool
_lt_t (Type T_Int) (Type T_Double) = Type T_Bool
_lt_t (Type t1) (Type t2) = Err $ "`<`: Illegal input type: LHS=" ++ (show t1) ++ ", RHS=" ++ (show t2)
...
_leq_t :: Wrd -> Wrd -> Wrd
_leq_t (Type T_Double) (Type T_Double) = Type T_Bool
_leq_t (Type T_Int) (Type T_Int) = Type T_Bool
_leq_t (Type T_Double) (Type T_Int) = Type T_Bool
_leq_t (Type T_Int) (Type T_Double) = Type T_Bool
_leq_t (Type t1) (Type t2) = Err $ "`>=`: Illegal input type: LHS=" ++ (show t1) ++ ", RHS=" ++ (show t2)
...
_and_t :: Wrd -> Wrd -> Wrd
_and_t (Type T_Bool) (Type T_Bool) = Type T_Bool
_and_t (Type t1) (Type t2) = Err $ "`&&`: Illegal input type: LHS=" ++ (show t1) ++ ", RHS=" ++ (show t2)

_and :: Op
_and = BinOp (\ (Bool a) (Bool b) ->  Bool (a && b))

_or_t :: Wrd -> Wrd -> Wrd
_or_t (Type T_Bool) (Type T_Bool) = Type T_Bool
_or_t (Type t1) (Type t2) = Err $ "`||`: Illegal input type: LHS=" ++ (show t1) ++ ", RHS=" ++ (show t2)

_or :: Op
_or = BinOp (\ (Bool a) (Bool b) ->  Bool (a || b))

_not_t :: Wrd -> Wrd
_not_t (Type T_Bool) = Type T_Bool
_not_t (Type t) = Err $ "`!`: Illegal input type: " ++ (show t)

_not :: Op
_not = UnOp (\ (Bool b) -> Bool (not b))

_head_t :: Wrd -> Wrd
_head_t (Type (T_List t)) = Type t
_head_t (Type t) = Err $ "head: Illegal input type: " ++ (show t)
...
_tail_t :: Wrd -> Wrd
_tail_t (Type (T_List t)) = Type (T_List t)
_tail_t (Type t) = Err $ "tail: Illegal input type: " ++ (show t)
...
_pop_t :: Wrd -> Wrd
_pop_t (Type (T_List t)) = Type (T_Tuple [t, T_List t])
_pop_t (Type t) = Err $ "pop: Illegal input type: " ++ (show t)
...
_isEmpty_t :: Wrd -> Wrd
_isEmpty_t (Type (T_List _)) = Type T_Bool
_isEmpty_t (Type t) = Err $ "isEmpty: Illegal input type: " ++ (show t)

_isEmpty :: Op
_isEmpty = UnOp (\ (List ls) -> Bool (ls == []))

_take_t :: Exp -> Wrd
_take_t (Type T_Int : (Type (T_List t) : [])) = Type (T_List t)
...
_seq_t :: Exp -> Wrd
_seq_t (Type T_Int : (Type T_Int : [])) = Type (T_List T_Int)
...
_map_t :: Exp -> Wrd
_map_t (Type T_Func : (Type (T_List t) : [])) = Type T_PreList
...
_fst_t :: Wrd -> Wrd
_fst_t (Type (T_Tuple (t : _))) = Type t
...
_snd_t :: Wrd -> Wrd
_snd_t (Type (T_Tuple (_ : (t : _)))) = Type t
